...
export function BlogList() {
  ...
  const posts = useMemo(() => {
    if (!activeTag) return allPostsForLang;
    return allPostsForLang.filter((p) => p.frontmatter.tags.includes(activeTag));
  }, [allPostsForLang, activeTag]);
      ...
      >
        ...
        {posts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-500">
            ...
            <p className="text-lg">
              {activeTag
                ? (lang === 'zh' ? '该标签下暂无文章' : 'No posts with this tag')
                        ...
                        {post.frontmatter.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {post.frontmatter.tags.map((tag) => (
                              <span
                                key={tag}
                                className="rounded-full border border-white/5 bg-white/[0.03] px-2 py-0.5 text-[11px] text-gray-500"
                              >
                                {tag}
                              </span>
                            ))}
