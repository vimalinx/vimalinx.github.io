...
export function Home() {
  ...
  const heroExitLocked = useRef(true);
    ...
    <div className="relative h-screen w-full overflow-hidden bg-black text-white selection:bg-purple-500/30">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-between px-6 py-6 md:px-12">
        <div className="flex items-center gap-6">
          <Link to="/" className="text-xl font-bold tracking-tighter mix-blend-difference">VIMALINX</Link>
          <Link
            to="/blog"
            className="hidden sm:inline-flex items-center text-sm font-medium text-gray-400 hover:text-white transition-colors mix-blend-difference"
          >
            {lang === 'zh' ? '博客' : 'Blog'}
          </Link>
        </div>
